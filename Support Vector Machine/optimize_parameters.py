...
##The 'searchspace' is a range of values for different parameters we want to optimize: We might have a guess for the different types or ranges of promising decision functions / gamma values, but do not know
##the optimal combination of parameters. That is why such a 'grid search' is performed.
fspace = {
...
max_evals = int(args.max_evals) ##Number of maximum evaluations / trials: The algorithm randomly picks a decision_function - gamma value pair, and checks the score based on a short training.
##                                The pair with the highest score gets picked for actual training.
current_eval = 1
train_history = []
